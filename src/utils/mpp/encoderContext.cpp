...
bool MppEncoderContext::applyConfig() {
    ...
    // 固定输入/输出帧率 (参考MPP测试代码第318-323行)
    set_cfg("rc:fps_in_flex", mCurCfg.rc_fps_in_flex);
    set_cfg("rc:fps_in_num", mCurCfg.rc_fps_in_num);  // 使用独立的输入帧率
    ...
    set_cfg("rc:drop_thd", 20);        /* 20% of max bps */
    set_cfg("rc:drop_gap", 1);         /* Do not continuous drop frame */
